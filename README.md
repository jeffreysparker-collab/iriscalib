# IrisCalib

Calibrated iris diameter measurement using a Aruco tags as a physical ruler.
Built with MediaPipe FaceMesh + WebGL. Runs entirely in the browser — no server, no data sent anywhere.

https://jeffreysparker-collab.github.io/iriscalib/

## Usage


1. Open on iPhone Safari — tap **Share → Add to Home Screen** for PWA install
2. Tap **Start Camera**
3. Hold credit card portrait mode (53.98 mm narrow) flat beside your LEFT eye, pushed into cheek.
4. Tap 4 points spanning the full card width on screen
5. Tap **Lock Scale**
6. Look at camera — tap **Record Left**  to record iris diameter
7. Repeat for Right eye

## Calibration note
Card must be at the **same focal plane as the eye** — hold it flat against your cheekbone/temple.

